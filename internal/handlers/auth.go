...
func Logout(c *gin.Context) {
    // 1. Extraer refresh token del body
    var body struct {
        RefreshToken string `json:"refresh_token"`
    }
    if err := c.ShouldBindJSON(&body); err != nil {
    ...
    _, err = db.Exec("DELETE FROM refresh_tokens WHERE token = ?", body.RefreshToken)
    if err != nil {
        c.JSON(http.StatusInternalServerError, gin.H{"error": "Error al eliminar token"})
        return
    }

    c.JSON(http.StatusOK, gin.H{"message": "Sesión cerrada con éxito"})
}
func GetProfile(c *gin.Context) {
    // Email del usuario obtenido del middleware
